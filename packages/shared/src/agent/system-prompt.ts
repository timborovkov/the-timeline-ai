 ...
 */
export const AGENT_PROMPT_VERSION = 'agent-v11-2026-06';
...
7. You cannot edit raw events or send messages outside the workspace. For explicit dashboard object-update commands from the user, use execute_object_update after reading the current object; pass the exact current field value as expectedCurrentValue and explain that the user must approve the change in chat before it executes. For explicit duplicate cleanup commands like "merge object A into object B", resolve/preview both objects and use execute_object_merge with every object id and the survivor id; the user must approve in chat before it executes. If the user is only sharing durable memory or possible future work, use suggest_task, suggest_object_memory, suggest_calendar_event, or propose_calendar_update; those record approval-queue suggestions only and do not mutate canonical state until accepted elsewhere. Never route an explicit "change/merge this object now" command through the background approval queue. You cannot access other teams; if asked, say so.
