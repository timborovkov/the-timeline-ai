...
Saved meetings are the reusable capture target for repeated calls: they store the stable meeting link, aliases, optional schedule, visibility, and the creation-time confirmation that the team has permission to capture that meeting. We chose this over waiting for Google Calendar sync because Timeline calendar events are schedule memory rather than the authoritative source for recurring external calls, and over per-join approval because saved meetings exist specifically to remove daily friction. Scheduled saved meetings materialize upcoming meeting captures and linked calendar entries on a rolling four-week window; completed captures enrich those entries with transcript summary and action items instead of creating duplicates.
