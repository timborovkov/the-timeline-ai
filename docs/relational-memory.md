...
2. **Relations are cheap, typed, and durable.** Join on provider ids,
   conversation keys, canonical URLs, artifact clusters, and accepted object
   links. Do not join on "happened nearby" or "embeds similarly."
3. **LLMs read bounded packs of already-related evidence.** They do not
   discover the graph. Semantic search may rank eligible rows and may answer
   questions. It does not admit a pulse into a durable proposal.
...
cluster. A workflow run attaches to the PR or SHA cluster. They become
supporting evidence when a later communication or captured-work review asks
for a pack. They do not start that review.
...
| First-class signal-class field on ingest | Not shipped | Classifier at write time, inspectable in metadata |
...
1. Classify by event, not by OAuth app.
...
- Classifier target: a shared signal-class helper used by the integration
  event writer and conversational ingest, stored on `source_metadata`, not
  another provider-wide skip set.
- Captured-work proposals: follow
  `packages/shared/src/integrations/github-task-proposals.ts` — coalesce by
  external object id, parse structured fields, match existing open tasks,
  write approval bundles.
...
- Lift the provider skip list into an event-level signal class.
...
- Let pulses into proposal packs only as supporting evidence when a hard join
  key already exists.
- Reuse the GitHub captured-work proposal template for Linear/Jira-style
  status and assignee fields without an LLM.
