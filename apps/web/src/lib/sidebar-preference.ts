...
export function persistSidebarExpanded(expanded: boolean) {
  ...
  try {
    ...
    document.cookie = `${SIDEBAR_COOKIE_KEY}=${value}; Path=/; Max-Age=${COOKIE_MAX_AGE_SECONDS}; SameSite=Lax${secure}`;
  } catch {
...
export const SIDEBAR_PREFERENCE_BOOTSTRAP = `(()=>{try{const s=localStorage.getItem('${SIDEBAR_STORAGE_KEY}');if(s!=='true'&&s!=='false')return;const k='${SIDEBAR_COOKIE_KEY}=',c=document.cookie.split('; ').find(v=>v.startsWith(k))?.slice(k.length);if(c!==undefined)return;document.cookie=k+s+'; Path=/; Max-Age=${COOKIE_MAX_AGE_SECONDS}; SameSite=Lax'+(location.protocol==='https:'?'; Secure':'');const n=document.cookie.split('; ').find(v=>v.startsWith(k))?.slice(k.length);if(n===s)location.reload()}catch{}})()`;
