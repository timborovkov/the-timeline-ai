...
test('onboarding checklist supports manual completion, dismissal, and reopening', async ({
  ...
  await expect(ownerPage.getByRole('heading', { name: 'Getting started' })).toBeVisible();
  ...
  await waitForOnboardingPatch(ownerPage, async () => {
    await ownerPage.getByRole('button', { name: 'Hide getting started' }).click();
  });
  await expect(ownerPage.getByRole('button', { name: 'Show getting started' })).toBeVisible();

  await ownerPage.reload();
  await expect(ownerPage.getByRole('button', { name: 'Show getting started' })).toBeVisible();

  await waitForOnboardingPatch(ownerPage, async () => {
    await ownerPage.getByRole('button', { name: 'Show getting started' }).click();
  });
  await expect(ownerPage.getByRole('heading', { name: 'Getting started' })).toBeVisible();
});
