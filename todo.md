...
- [x] Document processing worker: text / markdown / json / xml routes ship working end-to-end. PDF, DOCX, image (LLM-vision OCR) routes are scaffolded but stamp `processing_status='failed'` with a clear reason — adding extractors is a follow-up that touches one file. Chunker (~800 token target / ~120 overlap) in `packages/shared/src/chunk.ts`. (`apps/worker/src/workers/documentExtract.ts`.)
...
- [ ] PDF / DOCX / image (OCR) extractors. Schema + queue + chunk-embed pipeline are in place; the routing branch in `apps/worker/src/workers/documentExtract.ts::routeContentToText` currently returns null for these. Adding a route is mechanical.
