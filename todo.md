...
- [x] Bot lifecycle worker [`apps/worker/src/workers/meetingFinalize.ts`](apps/worker/src/workers/meetingFinalize.ts): when Recall reports the call is over (`bot.call_ended`, `bot.done`, or `transcript.done`), the status webhook flips the meeting to `processing` and enqueues finalize, which generates the summary, extracts action items, creates one consolidated `raw_events` row (full transcript + summary), creates a native calendar event titled like "Google Meet with Meeting Bot: Weekly planning", backfills `rawEventId` on all chunks, enqueues extract + embed for the meeting event plus a calendar-event embed when team-visible, records minutes (idempotent via `meeting_usage` unique index), and flips to `completed`. Late transcript chunks after finalization mark the meeting/calendar summaries stale, refresh the transcript raw event, and re-embed the affected calendar event.
...
- [x] UI surface — [`/app/meetings`](apps/web/src/app/app/meetings/page.tsx) lists recent meetings + current-month minutes against the team cap. [`ScheduleMeetingBotForm`](apps/web/src/components/meeting-forms.tsx) takes URL, optional title, visibility, and consent. Meeting detail page shows status, speaker-by-speaker transcript, summary, export controls, and (once finalised) extracted action items; finalized meeting timeline rows link back to the transcript detail.
...
- [x] Pending approvals UI: one cross-domain queue for suggested tasks/objects/object edits/calendar creates/calendar refinements/calendar cancels, with source citations, confidence/reason, pending/failed/resolved/all filters, accept/reject, accept-all, per-item accept/reject inside bundles, and contextual review on object/calendar pages.
...
- [x] Background suggestion worker: separate from the existing facts/entities extract worker. Non-conversational sources still read the raw event, facts/entities, recent context, and existing workspace state to extract commitments and create/dedupe/merge suggestion bundles. Slack and Telegram sources now debounce by source conversation and review a bounded conversation evidence window before proposing tasks, calendar changes, or object memory updates; cross-source context is limited to explicit object-backed links, deterministic fallback is disabled for conversation reviews, and `no_action` is a successful outcome when the conversation is ambiguous or contradicted.
...
- [x] **Retrieval + prompt foundation:** agent tools expose raw sender context
      ...
      them.
- [x] **Approval-backed memory writes:** extend bundled `agent_suggestions` for
...
- [x] **Prominent approval UX:** web chat renders inline approval cards for
      ...
      approvals.
- [x] **Regression coverage:** tests cover sender context hydration from
