...
- [ ] **Slack bot ingest, equivalent to Telegram:** Slack app/bot install with team + channel bindings, DM and channel message ingest, edit handling, user attribution, reaction ack on every ingested message, and `/ask <question>` or mention-based in-channel agent answers. Reuse the Telegram mental model and the same `withTeam` scope + `agent.askAgent` path; Slack should be a conversational bot surface, not only a passive integration feed.
