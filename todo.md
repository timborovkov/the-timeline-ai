...
- [ ] DB-outage silent drop on `/api/email/inbound`. Today every error path returns 200 so Postmark never retries. For transient infra (DB, S3, queue down) we should return 503 so Postmark retries. Wire a transient-vs-permanent error classifier or a DLQ queue.
